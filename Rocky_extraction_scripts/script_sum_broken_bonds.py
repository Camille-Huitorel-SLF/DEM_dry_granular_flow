...
class RockyDumper:
    ...
    def dump(self, dump_path: Path, index: int):
        if dump_path.is_dir():
            dump_path /= f"bed_{index}.json"
...
def main():
    ...
    user_process_bed = user_processes.GetProcess("bed")
    broken_bonds, seconds = [], []
    for index, timestep in enumerate(timesteps_picked):
        broken_bonds_t = np.sum(
            user_process_bed.GetGridFunction("Particle Broken Bonds").GetArray(
                time_step=timestep
            )
        )
        broken_bonds.append(broken_bonds_t)
        # timestep_second = f"{timestep.second:.3e}"
        seconds.append(timestep.second)
    df = {
        "seconds": seconds,
        "sum_broken_bonds": broken_bonds,
    }

    # dumper.log_dataframe("Particles", df, timestep)
    filename = run_outfolder / "broken_bonds.json"
